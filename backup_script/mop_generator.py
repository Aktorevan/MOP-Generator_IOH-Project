...
def CreateFile(name):
    ...
    #Select the column
    Table = data[data["MOP File Name"] == str(name)][['Relative NE ','Scope', 'NE Region','Time', 'Execution Date', 'Dependency Qty', 'Site Dependency', 'Impact Data Source']]
    
    #Filter based on region for creating folder
    Region = Table['NE Region'].unique()

    #Filter based on date for creating folder
    Date = Table['Execution Date'].unique()

    #Filter based on SOW for creating folder and selecting the template
    SOW = Table['Scope'].unique()
